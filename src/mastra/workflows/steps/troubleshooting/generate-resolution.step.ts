...
export const generateResolutionStep = createStep({
  ...
  description:
    "Generates the final resolution recommendation and diagnostic report",
  ...
  execute: async ({ inputData, getInitData }) => {
    ...
    const defectDescription =
      confirmedHypothesis?.description || initData.symptomDescription;
    ...
    let recommendations: string[] = [
      "Review defect and assess priority manually",
    ];

    if (
      "priority" in classificationResult &&
      "recommendations" in classificationResult
    ) {
    ...
    const priorityToMaintenanceLevel: Record<
      string,
      "L1" | "L2" | "L3" | "L4"
    > = {
      critical: "L3",
    ...
    if (confirmedHypothesis?.componentId) {
      ...
      requiredParts.push(
        ...(componentParts[confirmedHypothesis.componentId] || []),
      );
    }
    ...
    const report = {
      ...
      rootCause: {
        description:
          confirmedHypothesis?.description || "Unable to determine root cause",
