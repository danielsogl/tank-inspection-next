...
const gatherInformationStep = createStep({
  ...
  description:
    "Gathers inspection information from multiple sources in parallel",
  ...
  execute: async ({ inputData, requestContext }) => {
    ...
    tasks.push(
      queryInspectionTool
        .execute(
          { query, topK: 5, minScore: 0.5, useReranking: false },
    ...
    for (const { type, result } of results) {
      ...
      switch (type) {
        ...
        case "checkpoint":
          if (
            "found" in (result as Record<string, unknown>) &&
            (result as { found: boolean }).found
          ) {
            organized.checkpoint = (
              result as { checkpoint: unknown }
            ).checkpoint;
          }
          break;
        case "component":
...
const formatResponseStep = createStep({
  ...
  execute: async ({ inputData, getInitData }) => {
    const { searchResults, checkpoint, component, maintenance, sourcesUsed } =
      inputData;
    ...
    if (checkpoint) {
      ...
      summaryParts.push(
        `Checkpoint ${cp.checkpointNumber}${cp.checkpointName ? `: ${cp.checkpointName}` : ""}`,
      );
    }
    ...
    if (maintenance) {
      const maint = maintenance as {
        summary: { totalIntervals: number; dueIntervals?: number };
      };
      summaryParts.push(
        `${maint.summary.totalIntervals} maintenance intervals`,
      );
    ...
    return {
      summary,
      searchResults: searchResults as
        | Array<{
            content: string;
            sectionId: string;
            checkpointNumber?: number;
...
Do NOT use this for troubleshooting symptoms - use the troubleshooting workflow instead.`,
  inputSchema: z.object({
    ...
    checkpointNumber: z
      .number()
      .optional()
      .describe("Specific checkpoint number if known"),
    componentId: z
      .string()
      .optional()
      .describe("Component ID (mtu_mb873, renk_hswl354, turmdrehkranz)"),
    maintenanceLevel: z
      .enum(["L1", "L2", "L3", "L4"])
      .optional()
      .describe("Filter by maintenance level"),
    operatingHours: z
      .number()
      .optional()
      .describe("Current operating hours for maintenance queries"),
  }),
  outputSchema: z.object({
    summary: z.string(),
    searchResults: z
