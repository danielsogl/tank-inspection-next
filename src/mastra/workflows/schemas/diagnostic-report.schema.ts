...
export const diagnosticStepSchema = z.object({
  ...
  expectedResult: z.string().describe('What result is expected if functioning correctly'),
...
export const rootCauseSchema = z.object({
  ...
  componentId: z.string().optional().describe('Component ID if known (e.g., mtu_mb873)'),
});
...
export const resolutionSchema = z.object({
  priority: z.enum(['critical', 'high', 'medium', 'low', 'info']).describe('Defect priority'),
  maintenanceLevel: z.enum(['L1', 'L2', 'L3', 'L4']).describe('NATO maintenance level required'),
  recommendedAction: z.string().describe('Recommended repair/maintenance action'),
...
export const diagnosticReportSchema = z.object({
  ...
  timestamp: z.string().describe('ISO timestamp of when the diagnosis was performed'),
  sessionId: z.string().describe('Unique session identifier'),

  // Symptom input
  symptomDescription: z.string().describe('Original symptom description from user'),
  affectedSystems: z.array(z.string()).describe('Systems identified as potentially affected'),

  // Diagnosis path
  diagnosticSteps: z.array(diagnosticStepSchema).describe('Steps performed during diagnosis'),
  ...
  // Evidence
  evidenceTrail: z.array(z.string()).describe('Evidence gathered during diagnosis'),
});
...
export const troubleshootingInputSchema = z.object({
  symptomDescription: z.string().describe('Description of the symptom or issue'),
  vehicleId: z.string().default('leopard2').describe('Vehicle ID being diagnosed'),
  componentHint: z.string().optional().describe('Optional hint about which component may be affected'),
  requireApproval: z.boolean().default(false).describe('Whether to require human approval'),
});
