...
export type TriggerType = "event" | "calendar" | "operating_hours";
...
export const MAINTENANCE_INTERVALS: MaintenanceInterval[] = [
  {
    ...
    sections: ["A", "B", "F"],
    notes:
      "Vor jeder Ausfahrt und nach Rückkehr durchzuführen. Basis für Einsatzbereitschaft.",
  },
  {
    id: "L1-WEEKLY",
    level: "L1",
  ...
  },
  {
    ...
    sections: ["A", "B"],
    notes:
      "MTU-Wartungskonzept Level 1. Entspricht NATO AJP-4 Level 2 (Intermediate).",
  },
  {
    id: "L2-500H",
    level: "L2",
    ...
    sections: ["A", "B", "C"],
    notes:
      "MTU-Wartungskonzept Level 2. Umfangreiche Filterarbeiten und Inspektionen.",
  },
  {
    id: "L3-1000H",
    level: "L3",
  ...
  },
  {
    ...
    sections: ["A", "B", "C", "D", "E", "F"],
    notes:
      "Vor jedem Einsatz durchzuführen. Vollständige Einsatzbereitschaft herstellen.",
  },
  {
    ...
    sections: ["A", "B", "C", "D", "E", "F"],
    notes:
      "Nach jedem Einsatz. Fahrzeug wieder in Bereitschaft versetzen und Schäden erfassen.",
  },
  {
    id: "L2-ANNUAL",
    level: "L2",
    ...
    sections: ["A", "B", "C", "D", "E", "F"],
    notes:
      "Unabhängig von Betriebsstunden. Zeitabhängige Wartung und gesetzliche Prüfungen.",
  },
];

export const EXECUTOR_DESCRIPTIONS: Record<
  Executor,
  { de: string; en: string }
> = {
  ...
  mobile_repair_team: {
    de: "Feldwerft / Instandsetzungstrupp",
    en: "Mobile Repair Team",
  },
  depot_manufacturer: { de: "HIL / Hersteller", en: "Depot / Manufacturer" },
};

/**
 * Gets executor description by executor type.
 */
export function getExecutorDescription(executor: Executor): {
  de: string;
  en: string;
} {
  return EXECUTOR_DESCRIPTIONS[executor] || { de: executor, en: executor };
}

/**
 * Gets intervals filtered by level.
 */
export function getIntervalsByLevel(
  level: MaintenanceLevel,
): MaintenanceInterval[] {
  return MAINTENANCE_INTERVALS.filter((i) => i.level === level);
}

/**
 * Gets a specific interval by ID.
 */
export function getIntervalById(id: string): MaintenanceInterval | undefined {
  return MAINTENANCE_INTERVALS.find((i) => i.id === id);
}
